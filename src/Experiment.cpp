...
void Experiment::update(){
  ...
  double jump[3];
  fori(0,3){
    jump[i] = rng.nextd(-js, js);
    ps[3*p+i] += jump[i];
  }

  enn = enerNL(p, cellb);
    
  float P = exp(-beta*(enn-eno));
...
void Experiment::adjust_step(){
  ...
  if(ratio<TRUST){this->js *=0.99f;}
  else if(ratio>TRUST){ this->js *= 1.01f;}
  ntry = naccept = 0;
  outjs<<js<<" "<<lcell<<endl;
}
...
double Experiment::enerNL(uint index, uint &icell) const{
  double H = 0.0;
  fori(0, ps.size()/3){
    if(i!=index)H+= LJ(&ps[3*index], &ps[3*i]);
  }
